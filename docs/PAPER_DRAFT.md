...
The AI retrieval target regime removes this capacity-dominant term, because every transfer fits a single 4~KB host page, the SQ Engine emits one PRP pointer with the second set to zero, and the PRP-list region is never allocated. Per-tile SRAM in this regime ranges from 81~KB to 644~KB across the four synthesized configurations, which cover 16 or 64 queue pairs at queue depth 64 or 128. General-purpose workloads that also issue transfers larger than one page restore the PRP-list region, and per-tile SRAM then grows from 329~KB at 16 queue pairs and queue depth 64 to 2.6~MB at 64 queue pairs and queue depth 128. These four points bracket the queue resources a multi-million-IOPS poll-mode driver keeps outstanding, since the $Q_D$ slots per queue match the in-flight window SPDK sustains, so the sizing follows real deployment demand rather than a worst-case envelope. A tile that had to serve thousands of simultaneously active queue pairs would multiplex them hierarchically or replicate the tile rather than grow a single SRAM, since the synthesized configurations cover up to 64 queue pairs each.
