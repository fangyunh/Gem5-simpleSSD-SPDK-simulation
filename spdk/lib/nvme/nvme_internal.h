...
struct nvme_request {
	...
	/* Per-IO cycle breakdown timestamps (TSC ticks) */
	uint64_t			t_start;
	uint64_t			t_sqe_copy;
	uint64_t			t_doorbell;
	uint64_t			t_completion;
	uint64_t			t_end;
	uint64_t			t_poll_start;
	uint64_t			t_poll_cycles;

	/* Detailed submit path timestamps (TSC ticks) */
	...
	/* Detailed completion path timestamps (TSC ticks) */
	uint64_t			t_cqe_seen;
...
#if defined(SPDK_UNIT_TEST) && !defined(SPDK_NVME_IO_CYCLE_IMPL)
static inline uint64_t nvme_io_cycle_rdtsc(void)
{
	return 0;
}

static inline void nvme_io_cycle_record(struct nvme_request *req)
{
	(void)req;
}
#else
uint64_t nvme_io_cycle_rdtsc(void);
void nvme_io_cycle_record(struct nvme_request *req);
#endif
