...
CORE_ID=2                 # CPU Core to pin the workload to
CORE_MASK="0x4"           # Hex mask for Core 2
OUTPUT_DIR=${OUTPUT_DIR:-bdev_data}
OUTPUT_FILE="$OUTPUT_DIR/phase1_bdev_results.csv"
LOG_DIR="$OUTPUT_DIR/logs"
HIST_DIR="$OUTPUT_DIR/histograms"
ERROR_LOG="$OUTPUT_DIR/phase1_bdev_errors.log"
...
for bin in lspci perf python3; do
...
# Start bdevperf in background and wait for RPC socket
rm -f "$RPC_SOCK"
if [ -S "$RPC_SOCK" ]; then
...
echo "========================================================"
echo "Phase 1 (bdev) Complete. Results saved to $OUTPUT_FILE"
echo "========================================================"
