...
Options:
  ...
  --uncore-mode N         Uncore mode: 0=disabled 1=Mode_A 2=Mode_B (default: $UNCORE_MODE)
...
preflight_disk_image() {
  ...
  $sudo_cmd ls -la "$PREFLIGHT_MOUNT_DIR/$GUEST_REPO/scripts/phase1_4k/phase1_run.sh" 2>/dev/null | sed 's/^/  /' | tee -a "$LOG_FILE"

  if $sudo_cmd test -d "$PREFLIGHT_MOUNT_DIR/$GUEST_REPO" 2>/dev/null && \
     $sudo_cmd test -f "$PREFLIGHT_MOUNT_DIR/$GUEST_REPO/scripts/phase1_4k/phase1_run.sh" 2>/dev/null; then
    repo_found="$GUEST_REPO"
  else
    for p in $GUEST_REPO_CANDIDATES; do
      if $sudo_cmd test -d "$PREFLIGHT_MOUNT_DIR/$p" 2>/dev/null && \
         $sudo_cmd test -f "$PREFLIGHT_MOUNT_DIR/$p/scripts/phase1_4k/phase1_run.sh" 2>/dev/null; then
        repo_found="$p"
        break
      fi
    done
...
send_guest_cmds() {
  ...
  tmux_cmd send-keys -t "$target" "if [ -d '$GUEST_REPO' ] && [ -f '$GUEST_REPO/scripts/phase1_4k/phase1_run.sh' ]; then cd '$GUEST_REPO'; else for p in $GUEST_REPO_CANDIDATES; do if [ -f \"\$p/scripts/phase1_4k/phase1_run.sh\" ]; then cd \"\$p\"; break; fi; done; fi" C-m
  ...
  tmux_cmd send-keys -t "$target" "export UNCORE_MODE=$UNCORE_MODE" C-m
  tmux_cmd send-keys -t "$target" "./scripts/phase1_4k/phase1_run.sh" C-m
}
...
HOST_SHARE="__HOST_SHARE__"
{
  ...
  find_repo() {
    for p in "$REPO_HINT" $REPO_CANDIDATES; do
      if [ -d "$p" ] && [ -f "$p/scripts/phase1_4k/phase1_run.sh" ]; then
        echo "$p"
        return 0
      fi
    done
    # Fallback: scan for the script within a shallow depth
    found=$(find / -maxdepth 6 -path "*/scripts/phase1_4k/phase1_run.sh" 2>/dev/null | head -n1)
    if [ -n "$found" ]; then
...
export UNCORE_MODE=__UNCORE_MODE__
  ./scripts/phase1_4k/phase1_run.sh
...
run_auto() {
  ...
  if [ "$USE_READFILE" -eq 1 ]; then
    ...
    tmux_cmd new-session -d -s "$SESSION_NAME" -n boot "bash -lc \"KERNEL=$KERNEL DISK_IMAGE=$DISK_IMAGE MEM_SIZE=$MEM_SIZE SSD_CONFIG=$SSD_CONFIG CHECKPOINT_DIR=$CHECKPOINT_DIR READFILE_SCRIPT=$READFILE_SCRIPT VIO_9P=$VIO_9P VIO_9P_SET_ROOT=$VIO_9P HOST_SHARE='$HOST_SHARE' GEM5_DISABLE_COW=$DISABLE_COW $SHARED_SCRIPTS_DIR/boot_gem5.sh start; echo 'boot_gem5.sh exited'; exec bash\""
  else
    tmux_cmd new-session -d -s "$SESSION_NAME" -n boot "bash -lc \"KERNEL=$KERNEL DISK_IMAGE=$DISK_IMAGE MEM_SIZE=$MEM_SIZE SSD_CONFIG=$SSD_CONFIG CHECKPOINT_DIR=$CHECKPOINT_DIR VIO_9P=$VIO_9P VIO_9P_SET_ROOT=$VIO_9P HOST_SHARE='$HOST_SHARE' GEM5_DISABLE_COW=$DISABLE_COW $SHARED_SCRIPTS_DIR/boot_gem5.sh start; echo 'boot_gem5.sh exited'; exec bash\""
  fi
...
if [ "$BOOT" -eq 1 ]; then
  ...
  KERNEL="$KERNEL" DISK_IMAGE="$DISK_IMAGE" MEM_SIZE="$MEM_SIZE" SSD_CONFIG="$SSD_CONFIG" CHECKPOINT_DIR="$CHECKPOINT_DIR" \
    "$SHARED_SCRIPTS_DIR/boot_gem5.sh" start
fi

if [ "$CONSOLE" -eq 1 ]; then
  "$SHARED_SCRIPTS_DIR/console_gem5.sh" "$CONSOLE_PORT" "$CONSOLE_HOST"
fi
...
if [ "$STOP" -eq 1 ]; then
  "$SHARED_SCRIPTS_DIR/boot_gem5.sh" stop
fi
