...
# IO-Uncore Mode 2B host-side activation:
# When the driver passed --uncore-mode 2, the device-side IO-Uncore is in Mode B
# and the patched SPDK fast path will early-exit on the BAR0+0x2000 hint register
# only if SPDK_UNCORE_MODE_B=1 is set in the environment. Anything else leaves
# the SPDK patch dormant and behavior is byte-equivalent to vanilla.
UNCORE_MODE=${UNCORE_MODE:-0}
if [ "${UNCORE_MODE}" = "2" ]; then
    export SPDK_UNCORE_MODE_B=1
    echo "PHASE1_RUNSCRIPT_INFO: UNCORE_MODE=2 -> exporting SPDK_UNCORE_MODE_B=1 for spdk_nvme_perf"
else
            ...
            for QD in "${QUEUE_DEPTHS[@]}"; do
                for RUN_ID in $(seq 1 $REPEATS); do
                    ...
                    if [ $RC -ne 0 ] || ! echo "$CMD_OUTPUT" | grep -q "^Total"; then
                        ...
                        # SIGILL forensics: capture last kernel-trap line so we can
                        # objdump the offending PC. SIGILL exits as rc=132 (128+SIGILL).
                        if [ "$RC" -eq 132 ] && command -v dmesg >/dev/null 2>&1; then
                            {
                                echo "---- dmesg trap excerpt ----"
                                dmesg 2>/dev/null | grep -E "trap|invalid opcode|spdk_nvme_perf" | tail -20 || true
                                echo "---------------------------"
                            } >> "$RUN_LOG"
                        fi
                        continue
                    ...
                    if [ -f "$CYCLE_OUT" ]; then
                        read SUBMIT_NS COMPLETE_NS PREAMBLE_NS TR_ALLOC_NS XLATE_NS CMD_NS FENCE_NS DB_NS CQE_NS TR_LOOKUP_NS FREE_NS FREE_LIB_NS FREE_CB_NS FREE_TOTAL_NS <<EOF
$(python3 - <<PY
import csv
import math
