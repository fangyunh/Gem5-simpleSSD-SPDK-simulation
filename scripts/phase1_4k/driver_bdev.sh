...
#   ./scripts/phase1_4k/driver_bdev.sh --auto \
#     --cores "2" \
#     --qpairs "1" \
#     --qd "16 32" \
#     --ios "4096 16384" \
...
send_guest_cmds() {
  ...
  tmux send-keys -t "$target" "./scripts/phase1_4k/phase1_bdev.sh" C-m
}
...
run_auto() {
  ...
  tmux new-session -d -s "$SESSION_NAME" -n boot "bash -lc \"$SHARED_SCRIPTS_DIR/boot_gem5.sh start; echo 'boot_gem5.sh exited'; exec bash\""
  tmux pipe-pane -t "$SESSION_NAME:0.0" -o "cat >> '$LOG_FILE'"

  tmux set-option -t "$SESSION_NAME" remain-on-exit on

  tmux split-window -t "$SESSION_NAME:0.0" -v "bash -lc \"while true; do $SHARED_SCRIPTS_DIR/console_gem5.sh $CONSOLE_PORT $CONSOLE_HOST && break; echo 'console exited, retrying in 2s'; sleep 2; done; exec bash\""
...
./scripts/phase1_4k/phase1_bdev.sh
EOF
fi
